...
@onready var hitbox_ataque = $dbox
@onready var shape_hitbox = $dbox/CollisionShape2D
...
func attack_1():
	if Input.is_action_just_pressed("ataque1") and is_on_floor() and not ataque_1:
		...
		velocity.x = 0
		shape_hitbox.disabled = false  # Activar hitbox


func _on_animation_finished():
	if animated_sprite.animation == "attack_1":
		ataque_1 = false
		shape_hitbox.disabled = true  # Desactivar hitbox
